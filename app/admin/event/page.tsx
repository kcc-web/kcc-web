  ...
  return (
    <div className="p-6">
      ...
      <div>
        ...
        {events.map((ev) => (
          <div key={ev.id} className="border p-4 mb-3 rounded">
            <div className="font-bold text-lg">{ev.title}</div>
            <div className="text-sm text-gray-600">{ev.date} @ {ev.location}</div>
            <p className="text-sm">{ev.description}</p>
            <div className="mt-2 space-x-4">
              <button className="text-blue-600 underline" onClick={() => setForm(ev)}>編集</button>
              <button className="text-red-600 underline" onClick={() => handleDelete(ev.id)}>削除</button>
            </div>
          </div>
        ))}
      </div>
